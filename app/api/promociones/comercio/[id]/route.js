...
export async function GET(request, { params }) {
    ...
    const { id_comercio } = params; // Obtener el userId de los parámetros de la URL

    try {
        // Buscar todas las promociones que coincidan con el userId y hacer populate del producto
        const promociones = await Promocion.find({ productoId: id_comercio }).populate('productoId', 'nombre');

        // Comprobar si se encontraron promociones
        if (promociones.length === 0) {
            return NextResponse.json({ message: "No se encontraron promociones para este comercio." }, { status: 404 });
        }
